...
def _pyrodigal_job(args_tuple):
    ...
    # Initialise finder based on --gene_caller
    try:
    ...
    for faa_file in faa_files:
        ...
        with open(prodigal_faa) as fh:
            for line in fh:
                ...
                # Detect edge-partial genes (start_type=Edge)
                # Prodigal starts from contig edge — only end coordinate is reliable
                ...
                # 4. Edge-partial: start_type=Edge — match on end only (no start tol)
                #    Prodigal uses contig edge as start; Bakta finds real start codon
...
def main():
    ...
    if args.fna_dir:
        ...
        faa_dir,gff_dir_path=run_prodigal(fna_files,out_dir,meta_mode=args.meta,
